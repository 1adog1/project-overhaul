<?php
    ...
    if (file_exists(__DIR__ . "/config.ini")) {
        ...
        //EVE AUTHENTICATION CONFIGURATION
        $configVariables["Client ID"] = $ENVS["ENV_OVERHAUL_EVE_CLIENT_ID"];
        $configVariables["Client Secret"] = $ENVS["ENV_OVERHAUL_EVE_CLIENT_SECRET"];
        $configVariables["Client Scopes"] = $ENVS["ENV_OVERHAUL_EVE_CLIENT_SCOPES"] ?? "esi-search.search_structures.v1";
        $configVariables["Default Scopes"] = $ENVS["ENV_OVERHAUL_EVE_DEFAULT_SCOPES"] ?? "esi-search.search_structures.v1";
        $configVariables["Client Redirect"] = $ENVS["ENV_OVERHAUL_EVE_CLIENT_REDIRECT"];
        $configVariables["Client Contact Info"] = $ENVS["ENV_OVERHAUL_EVE_CLIENT_CONTACT_INFO"];
        $configVariables["Auth Type"] = $ENVS["ENV_OVERHAUL_EVE_AUTH_TYPE"] ?? "Neucore";
        $configVariables["Super Admins"] = explode(",", str_replace(" ", "", $ENVS["ENV_OVERHAUL_EVE_SUPER_ADMINS"]));

        //NEUCORE AUTHENTICATION CONFIGURATION
        $configVariables["NeuCore ID"] = $ENVS["ENV_OVERHAUL_NEUCORE_APP_ID"] ?? NULL;
        $configVariables["NeuCore Secret"] = $ENVS["ENV_OVERHAUL_NEUCORE_APP_SECRET"] ?? NULL;
        $configVariables["NeuCore URL"] = $ENVS["ENV_OVERHAUL_NEUCORE_APP_URL"] ?? NULL;

        //DATABASE SERVER CONFIGURATION
        $configVariables["Database Server"] = $ENVS["ENV_OVERHAUL_DATABASE_SERVER"] . ":" . $ENVS["ENV_OVERHAUL_DATABASE_PORT"];
        $configVariables["Database Username"] = $ENVS["ENV_OVERHAUL_DATABASE_USERNAME"];
        $configVariables["Database Password"] = $ENVS["ENV_OVERHAUL_DATABASE_PASSWORD"];

        //DATABASE NAME CONFIGURATION
        $configVariables["Database Name"] = $ENVS["ENV_OVERHAUL_DATABASE_NAME"];

        //SITE CONFIGURATION
        $configVariables["Auth Cookie Name"] = $ENVS["ENV_OVERHAUL_WEBSITE_AUTH_COOKIE"] ?? "AuthID";
        $configVariables["Session Time"] = (int)($ENVS["ENV_OVERHAUL_WEBSITE_SESSION_TIME"] ?? 86400);
        $configVariables["Auth Cache Time"] = (int)($ENVS["ENV_OVERHAUL_WEBSITE_AUTH_CACHE_TIME"] ?? 0);
        $configVariables["Store Visitor IPs"] = boolval(($ENVS["ENV_OVERHAUL_WEBSITE_STORE_IPS"] ?? 0));
