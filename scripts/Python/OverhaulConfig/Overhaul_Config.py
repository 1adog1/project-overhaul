...
class Config:
    ...
    def __init__(self):
        ...
        if config_file.exists():
            ...
            self.database = Database(
                server = os.environ["ENV_OVERHAUL_DATABASE_SERVER"], 
                port = os.environ["ENV_OVERHAUL_DATABASE_PORT"], 
                username = os.environ["ENV_OVERHAUL_DATABASE_USERNAME"], 
                password = os.environ["ENV_OVERHAUL_DATABASE_PASSWORD"], 
                name = os.environ["ENV_OVERHAUL_DATABASE_NAME"]
            )

            self.eve_auth = EveAuth(
                ClientID = os.environ["ENV_OVERHAUL_EVE_CLIENT_ID"], 
                ClientSecret = os.environ["ENV_OVERHAUL_EVE_CLIENT_SECRET"], 
                ClientScopes = os.environ["ENV_OVERHAUL_EVE_CLIENT_SCOPES"] if "ENV_OVERHAUL_EVE_CLIENT_SCOPES" in os.environ else "esi-search.search_structures.v1", 
                DefaultScopes = os.environ["ENV_OVERHAUL_EVE_DEFAULT_SCOPES"] if "ENV_OVERHAUL_EVE_DEFAULT_SCOPES" in os.environ else "esi-search.search_structures.v1", 
                ClientRedirect = os.environ["ENV_OVERHAUL_EVE_CLIENT_REDIRECT"], 
                ClientContactInfo = os.environ["ENV_OVERHAUL_EVE_CLIENT_CONTACT_INFO"], 
                AuthType = os.environ["ENV_OVERHAUL_EVE_AUTH_TYPE"] if "ENV_OVERHAUL_EVE_AUTH_TYPE" in os.environ else "Neucore", 
                SuperAdmins = os.environ["ENV_OVERHAUL_EVE_SUPER_ADMINS"]
            )
